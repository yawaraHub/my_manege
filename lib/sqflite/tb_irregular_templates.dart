...
class IrregularTemplatesDao {
  ...
  Future<void> _createTableIfNotExists(Database db) async {
    final tableExists =
        await db.rawQuery("SELECT 1 FROM irregular_templates LIMIT 1");
  ...
  Future<int> update(Map<String, dynamic> row) async {
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.update('irregular_templates', row,
  ...
  Future<int> delete(int id) async {
    ...
    await _createTableIfNotExists(db);
    return await db
        .delete('irregular_templates', where: 'id = ?', whereArgs: [id]);
  }
}
