...
class SchedulesDao {
  ...
  Future<int> update(Map<String, dynamic> row) async {
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.update('schedules', row, where: 'id = ?', whereArgs: [row['id']]);
  }
  ...
  Future<List<Map<String, dynamic>>> getOneDaySchedule(String dateToSearch)async{
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.rawQuery("SELECT * FROM schedules WHERE start_at LIKE '$dateToSearch%'");
  }
}
