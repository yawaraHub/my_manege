...
class RegularTemplatesDao {
  ...
  Future<void> _createTableIfNotExists(Database db) async {
    final tableExists = await db.rawQuery("SELECT 1 FROM regular_templates LIMIT 1");
  ...
  Future<int> update(Map<String, dynamic> row) async {
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.update('regular_templates', row, where: 'id = ?', whereArgs: [row['id']]);
  }
  ...
  Future<int> delete(int id) async {
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.delete('regular_templates', where: 'id = ?', whereArgs: [id]);
  }
}
