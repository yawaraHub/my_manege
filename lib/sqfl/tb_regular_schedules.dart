...
class RegularSchedulesDao {
  ...
  Future<void> _createTableIfNotExists(Database db) async {
    final tableExists = await db.rawQuery("SELECT 1 FROM regular_schedules LIMIT 1");
  ...
  Future<int> update(Map<String, dynamic> row) async {
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.update('regular_schedules', row, where: 'id = ?', whereArgs: [row['id']]);
  }
  ...
  Future<int> delete(int id) async {
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.delete('regular_schedules', where: 'id = ?', whereArgs: [id]);
  }

  //regular_schedulesのidを渡すことでその日毎のスケジュールを返す[[1日目][2日目]]
  Future<List<List<Map<String,dynamic>>>> getSameRegularId(int parentId)async{
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    List<List<Map<String,dynamic>>> result = [];
    List<Map<String,dynamic>>parentData = await db.rawQuery('SELECT * FROM regular_templates WHERE id =?',[parentId]);

    for(int i = 1;i <= parentData[0]['day']; i++){
      result.add(await db.rawQuery('SELECT * FROM regular_schedules WHERE day = ?', [i]));
    }
    return result;
  }
}
