...
class IrregularSchedulesDao {
  ...
  Future<void> _createTableIfNotExists(Database db) async {
    final tableExists = await db.rawQuery("SELECT 1 FROM irregular_schedules LIMIT 1");
  ...
  Future<int> update(Map<String, dynamic> row) async {
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.update('irregular_schedules', row, where: 'id = ?', whereArgs: [row['id']]);
  }
  ...
  Future<int> delete(int id) async {
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.delete('irregular_schedules', where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<List<Map<String,dynamic>>> getSameParentId(int parentId)async{
    final db = await dbHelper.database;
    await _createTableIfNotExists(db);
    return await db.rawQuery('SELECT *FROM irregular_schedules WHERE irregular_template_id = ?', [parentId]);
  }
}
